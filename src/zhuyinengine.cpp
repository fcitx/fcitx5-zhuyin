...
void ZhuyinState::keyEvent(KeyEvent &keyEvent) {
  ...
  if (buffer_.empty()) {
    if (key.check(*engine_->config().quickphraseKey) &&
      ...
      if (!output.empty() && !altOutput.empty()) {
        ...
        engine_->quickphrase()->call<IQuickPhrase::trigger>(ic, "", "", "", "",
                                                            Key());
      }
  ...
  if (c <= std::numeric_limits<signed char>::max() && !charutils::isprint(c)) {
    if (!buffer_.empty()) {
      keyEvent.filterAndAccept();
    }
    return;
...
void ZhuyinState::updateUI(bool showCandidate) {
  ...
  if (showCandidate) {
    ...
    buffer_.showCandidate([this, &candidateList](
                              std::unique_ptr<ZhuyinCandidate> candidate) {
      candidate->connect<ZhuyinCandidate::selected>([this]() { updateUI(); });
